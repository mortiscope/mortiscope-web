...
interface ResultsRecalculateModalProps {
  ...
  /** Callback function to execute when recalculation is successfully initiated. */
  onRecalculationStart: () => void;
  /** Indicates if recalculation is currently in progress. */
 ...
 */
export const ResultsRecalculateModal = ({
  caseId,
  isOpen,
  onOpenChange,
  onRecalculationStart,
  isRecalculating = false,
}: ResultsRecalculateModalProps) => {
  // Hook to trigger the server action for starting the recalculation.
  const { mutate, isPending } = useMutation({
    mutationFn: requestRecalculation,
    onSuccess: (data) => {
      if (data.success) {
        // Call the parent's callback function to start the polling.
        onRecalculationStart();
      }
    },
    onError: () => {
  ...
  // Determine if the modal should be prevented from closing
  const shouldPreventClose = isPending || isRecalculating;

  return (
    <Dialog open={isOpen} onOpenChange={(open) => !shouldPreventClose && onOpenChange(open)}>
        ...
        >
          ...
          <motion.div variants={itemVariants} className="shrink-0 px-6 pt-4 pb-6">
            <DialogFooter className="flex w-full flex-row gap-3">
              <div className={cn("flex-1", shouldPreventClose && "cursor-not-allowed")}>
                <Button
                  variant="outline"
